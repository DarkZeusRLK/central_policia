...
// Salas criadas antes do recurso de reordenar não têm o campo `order` — preenche uma vez com a
// ordem em que já apareciam (por data de criação), sem mexer em quem já tem `order` definido.
export async function ensureRoomOrder(collection, rooms) {
  const missing = rooms.filter((room) => typeof room.order !== "number");
  if (!missing.length) return rooms;

  let nextOrder = rooms.reduce((max, room) => (typeof room.order === "number" ? Math.max(max, room.order + 1) : max), 0);
  const bulkOps = missing.map((room) => ({
    updateOne: { filter: { slug: room.slug }, update: { $set: { order: nextOrder++ } } },
  }));
  await collection.bulkWrite(bulkOps);

  let cursor = rooms.reduce((max, room) => (typeof room.order === "number" ? Math.max(max, room.order + 1) : max), 0);
  missing.forEach((room) => { room.order = cursor++; });
  return rooms;
}
