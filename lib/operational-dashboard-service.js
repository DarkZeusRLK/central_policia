...
// O schema real informado atualmente possui apenas dois formatos relevantes:
...
//
// Caso os nomes das coleções mudem no futuro, ajuste opcionalmente:
// - MONGODB_COLLECTION_POLICE_STATS
// - MONGODB_COLLECTION_RECRUITMENTS
//
// Esta integração permanece 100% em leitura.
...
export async function getOperationalDashboardData() {
  ...
  const [policeStats, recruitments] = await Promise.all([
  ...
  if (!policeStats.available) {
    warnings.push(
      "A coleção com estatísticas policiais não foi encontrada automaticamente. Ajuste MONGODB_COLLECTION_POLICE_STATS se necessário.",
    );
  }
  if (!recruitments.available) {
    warnings.push(
      "A coleção de recrutamentos não foi encontrada automaticamente. Ajuste MONGODB_COLLECTION_RECRUITMENTS se necessário.",
    );
  }
  ...
  return {
    ...
    metrics: {
      totalArrests: policeStats.totals.prisoes,
