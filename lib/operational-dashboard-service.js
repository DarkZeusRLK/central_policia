...
const guildMemberProfileCache =
  globalThis.__revoadaGuildMemberProfileCache || new Map();

globalThis.__revoadaArrestsCountCache = arrestsCountCache;
globalThis.__revoadaGuildMemberProfileCache = guildMemberProfileCache;
...
async function resolveGuildMemberProfile(discordId, botToken, guildId) {
  const id = String(discordId || "").trim();
  if (!id || !botToken || !guildId) return null;

  const cached = guildMemberProfileCache.get(id);
  ...
  try {
    ...
    guildMemberProfileCache.set(id, {
      profile,
      createdAt: Date.now(),
    });
...
async function enrichRankingWithGuildProfiles(items, botToken, guildId) {
  if (!Array.isArray(items) || !items.length || !botToken || !guildId) {
    return Array.isArray(items) ? items : [];
  }

  return Promise.all(
    items.map(async (item) => {
      const profile = await resolveGuildMemberProfile(
        item.discordId,
        botToken,
        guildId,
      );
      ...
      return {
        ...item,
        officer: profile.officer || item.officer,
...
export async function getOperationalDashboardData() {
  ...
  const [
    topArrestOfficers,
    topBlitzOfficers,
    topMarkedActionOfficers,
    topCatOperationOfficers,
    topRecruitmentOfficers,
  ] = await Promise.all([
    enrichRankingWithGuildProfiles(policeStats.rankings.prisoes, botToken, guildId),
    enrichRankingWithGuildProfiles(policeStats.rankings.blitz, botToken, guildId),
    enrichRankingWithGuildProfiles(
      policeStats.rankings.acoesMarcadas,
      botToken,
      guildId,
    ),
    enrichRankingWithGuildProfiles(
      policeStats.rankings.operacoesCat,
      botToken,
      guildId,
    ),
    enrichRankingWithGuildProfiles(recruitments.ranking, botToken, guildId),
  ]);

  const warnings = [];
  if (!policeStats.available) {
    warnings.push(
      "Os dados operacionais de apoio não puderam ser lidos nesta atualização.",
    );
  }
  if (!recruitments.available) {
    warnings.push(
      "Os dados de recrutamento não puderam ser lidos nesta atualização.",
    );
  }
  if (!arrestMetrics.available) {
    warnings.push(
      "O total de prisões não pôde ser atualizado com os registros atuais.",
    );
  }
