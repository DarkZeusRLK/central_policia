export default async function handler(req, res) {
  const { DISCORD_BOT_TOKEN, DIRETORES_PRF_IDS } = process.env;

  if (!DISCORD_BOT_TOKEN || !DIRETORES_PRF_IDS) {
    return res
      .status(500)
      .json({ error: "Configuração faltando no .env (DIRETORES_PRF_IDS)" });
  }
  ...
  try {
    ...
    console.error(error);
    return res.status(500).json({ error: "Erro ao buscar diretores PRF" });
  }
}
